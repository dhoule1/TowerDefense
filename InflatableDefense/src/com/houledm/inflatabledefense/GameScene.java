...
public class GameScene extends BaseScene implements IOnSceneTouchListener, IScrollDetectorListener, IPinchZoomDetectorListener{
	...
	public GameScene(MapType type) {	
		...
		activity = InflatableDefenseActivity.getSharedInstance();
	...
	public void showRedScreen() {
		...
		float difference = (camera.getZoomFactor() == 1.0f) ? 1.0f : 1.5f;
		redScreen.setY(-camera.getHeight()/3  - (GameMap.getTileSize()*camera.getZoomFactor() * difference));
		
		panel.attachChild(redScreen);
	...
	private void displayWinScreen() {
		...
		editor.putBoolean((gameMap.getMapID()+1)+"",true);
		editor.commit();
	}
	...
	public boolean onSceneTouchEvent(final Scene pScene, final TouchEvent pSceneTouchEvent) {
  	...
  	if (pSceneTouchEvent.isActionMove()) {
			...
			if (tClass != null && !towerMove) {
				...
				if (dragTower != null) {
					dragTower.getEntity().setZIndex(1);
					dragTower.getEntity().setScale(0.5f);
					SubMenuManager.setReticalPosition(dragTower.getEntity().getX()-dragTower.getEntity().getWidthScaled()/3.5f,
							dragTower.getEntity().getY()-dragTower.getEntity().getHeightScaled()/3.5f);
	...
	public void onBackKeyPressed() {
		if (!disableBackButton) toggleInGameMenu();
	}
